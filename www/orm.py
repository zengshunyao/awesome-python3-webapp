...
class Field(object):
    ...
    def __str__(self):
        if self.primary_key:
            return '<%s,%s:%s primarykey>' % (self.__class__.__name__, self.column_type, self.name);
        else:
...
# 模型
# 当我们传入关键字参数metaclass时，魔术就生效了，它指示Python解释器在创建Model时，要通过ModelMetaclass.__new__()来创建
